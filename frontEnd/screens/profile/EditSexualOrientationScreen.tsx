...
const EditSexualOrientationScreen = () => {
  ...
  const [sexualOrientationTemp, setsexualOrientationTemp] = useState<string[]>(
    sexualOrientation ? sexualOrientation : [],
  );

  const dispatch = useDispatch();

  const sexualOrientationField = questionsList.find(
    field => field.id === 13,
  ) as {question: string; id: number; answers: string[]};
  ...
  useEffect(() => {
    sexualOrientationTemp.length != 0
      ? dispatch(
      ...
      : dispatch(
          EditProfileActions.updateUserProfile('sexualOrientation', undefined),
        );
  }, [sexualOrientationTemp]);
