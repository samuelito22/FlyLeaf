...
const daySuffix = (day: number) => {
  ...
  const joinedDate = useMemo(() => {
    return `Joined on ${joinedDateObj.getDate()}${daySuffix(joinedDateObj.getDate())} of ${monthNames[joinedDateObj.getMonth()]} ${joinedDateObj.getFullYear()}`
  ...
  const FieldOfTable = React.memo(({feature, free, pro}: {feature:string, free: boolean, pro: boolean}) => {
    return (
  ...
  return (
    <SafeContainer>
      ...
      <ScrollView contentContainerStyle={styles.scrollViewContainer}>
        <Image source={{uri:userProfile.user.profile.pictures[1]}} style={styles.profilePicture} />
        <View style={{flex: 0.6}}>
      ...
      <View style={[styles.planCard, userProfile.user.appActivity.connects > 0 && {borderColor: "#fda831"}]}>
        ...
        <View  style={styles.planCard_textContainer}><Text style={styles.planCard_text}><Text style={{...themeText.bodyBoldThree, color: THEME_COLORS.dark}}>{userProfile?.user.appActivity.connects ? userProfile.user.appActivity.connects : 0}</Text> Connects</Text></View>
    <Button.ButtonImage imgUrl={icons.plus} contentContainerStyle={{position:'absolute', top: -8, right: -8, borderWidth: 0.2, borderRadius: 500, borderColor: THEME_COLORS.tertiary}} width={30} height={30} tintColor={THEME_COLORS.tertiary}/>
      ...
      <View style={[styles.planCard, userProfile.user.appActivity.isPremiumUser && {borderColor: "green"}]}>
        ...
        <View  style={styles.planCard_textContainer}><Text style={styles.planCard_text}>Pro membership</Text></View>
    <Button.ButtonImage imgUrl={icons.plus} contentContainerStyle={{position:'absolute', top: -8, right: -8, borderWidth: 0.2, borderRadius: 500, borderColor: THEME_COLORS.tertiary}} width={30} height={30} tintColor={THEME_COLORS.tertiary}/>
      </View>
      </View> 
...
inactiveText: {
  ...
  profilePicture: {resizeMode:'cover', width: 150,  aspectRatio: 1, borderRadius: 500, marginBottom: 5, alignSelf:'center' }
});
