...
const EditLanguageScreen = () => {
  ...
  const languages = useSelector(
    (state: TYPES.AppState) => state.editUserReducer.languages,
  );
  const [languagesTemp, setLanguagesTemp] = useState<string[]>(
    languages ? languages : [],
  );
  ...
  const languagesField = useMemo(() => {
    return questionsList.find(field => field.id === 14) as {
      question: string;
      id: number;
      answers: {code: string; name: string}[];
    };
  }, [questionsList]);

  const interestButtonHandlePress = useCallback(
    (name: string) => {
      if (languagesTemp.includes(name)) {
        setLanguagesTemp(languagesTemp.filter((pref: string) => pref !== name));
      } else if (languagesTemp.length < 5) {
        setLanguagesTemp([...languagesTemp, name]);
      }

      languagesTemp.length != 0
        ? dispatch(
            EditProfileActions.updateUserProfile(
              'languages',
              Array.from(languagesTemp),
            ),
          )
        : dispatch(
            EditProfileActions.updateUserProfile('languages', undefined),
          );
    },
    [languagesTemp],
  );
