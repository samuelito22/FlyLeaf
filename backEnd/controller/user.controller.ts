...
async function updateUserLocation(req: express.Request, res: express.Response) {
  ...
  try {
    ...
    if (!user) {
      return res
        .status(400)
        .json({ type: "error", message: USER_NOT_FOUND_ERR });
    }
...
async function getUserLocation(req: express.Request, res: express.Response) {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ type: "success", location: user.location.lastLocation });
  } catch (error) {
...
async function getUserProfile(req: express.Request, res: express.Response) {
  ...
  try {
    ...
    const spotifyData = await Spotify.findOne({
      _id: user.profile.spotify.spotify_id,
    });
...
async function updateUserProfile(req: express.Request, res: express.Response) {
  ...
  try {
    ...
    else unsetFields["profile.bio"] = 1;
    if (gender !== undefined) updateFields["profile.gender"] = gender;
    else unsetFields["profile.gender"] = 1;
    if (jobTitle !== undefined) updateFields["profile.jobTitle"] = jobTitle;
    else unsetFields["profile.jobTitle"] = 1;
    if (company !== undefined) updateFields["profile.company"] = company;
    else unsetFields["profile.company"] = 1;
    if (height !== undefined) updateFields["profile.height"] = height;
    else unsetFields["profile.height"] = 1;
    if (pictures !== undefined) updateFields["profile.pictures"] = pictures;
    else unsetFields["profile.pictures"] = 1;

    if (languages !== undefined)
      updateFields["interests.languages"] = languages;
    else unsetFields["interests.languages"] = 1;
    if (covidVaccination !== undefined)
      updateFields["interests.covidVaccination"] = covidVaccination;
    else unsetFields["interests.covidVaccination"] = 1;
    if (ethnicity !== undefined)
      updateFields["interests.ethnicity"] = ethnicity;
    else unsetFields["interests.ethnicity"] = 1;
    if (additionalInformation !== undefined)
      updateFields["interests.additionalInformation"] = additionalInformation;
    else unsetFields["interests.additionalInformation"] = 1;

    if (sexualOrientation !== undefined)
      updateFields["preferences.sexualOrientation"] = sexualOrientation;
    else unsetFields["preferences.sexualOrientation"] = 1;
    ...
    const updatedUser = await User.findOneAndUpdate({ _id: uid }, updateQuery, {
      new: true,
    });
    ...
    if (user.profile.instagram?.isConnected) {
      const InstagramData = await instagramModel.findOne({
        _id: user.profile.instagram.instagram_id,
      });

      combinedProfile = {
        ...combinedProfile,
        instagram: InstagramData?.images,
      };
    }

    if (user.profile.spotify?.isConnected) {
      const spotifyData = await SpotifyModel.findOne({
        _id: user.profile.spotify.spotify_id,
      });
...
async function initUserProfile(req: express.Request, res: express.Response) {
  ...
  try {
    ...
    if (
      ...
      Math.abs(
        locationData.coordinates.longitude -
          user.location.lastLocation.coordinates.latitude
      ) > 0.01 ||
      Math.abs(
        locationData.coordinates.longitude -
          user.location.lastLocation.coordinates.latitude
    ...
    if (user.profile.spotify?.isConnected) {
      ...
      await SpotifyServices.fetchTopArtists(
        result.accessToken,
        result.spotify_id
      );
    ...
    if (user.profile.instagram?.isConnected) {
      ...
      if (!InstagramData)
        return res
          .status(404)
          .json({ type: "error", message: "Instagram data not found" });
      ...
      if (expiryDate && accessToken && currentDate > expiryDate) {
        const result = await InstagramServices.refreshInstagramToken(
          accessToken
        );
      ...
      if (accessToken)
        await InstagramServices.fetchInstagramImages(
          accessToken,
          instagram_id
        ).catch((e) => console.log(e));
